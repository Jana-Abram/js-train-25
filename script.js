...
async function printFiveItems() {
  ...
  console.log("Почнемо виведення чисел через паузу:");

   for await (const value of gen) {
     console.log(value);
     if (value === 4) {
      console.log("Зупиняємо виведення.");
      break;
    }
  }
}
...
async function displayData() {
  console.log("Почнемо отримання даних:");
  const dataGenerator = gatherData();
  for (let i = 0; i < 3; i++) {
    const result = await dataGenerator.next();
    console.log("Отримані дані:", result.value);
  }
}
